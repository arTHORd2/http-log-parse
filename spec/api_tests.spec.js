...
describe("Count frequent URLs endpoint", function () {
  it("should get the number of frequent URLs specified by parameter", async function () {
    ...
    const data = await r.json()
    expect(JSON.stringify(data.commonUrls)).toEqual(JSON.stringify(['/docs/manage-websites/', '/intranet-analytics/']))
  })
  it("should default to 3 if no count is queried", async function () {
    const r = await fetch("http://localhost:3000/top_urls")
    const data = await r.json()
    expect(JSON.stringify(data.commonUrls)).toEqual(JSON.stringify(['/docs/manage-websites/', '/intranet-analytics/', 'http://example.net/faq/']))
  })
  it("should return an empty list if count is 0", async function () {
...
describe("Count frequent IPs endpoint", function () {
  it("should get the number of frequent IPs specified by parameter", async function () {
    ...
    const data = await r.json()
    expect(JSON.stringify(data.commonIps)).toEqual(JSON.stringify(['168.41.191.40', '177.71.128.21']))
  })
  it("should default to 3 if no count is queried", async function () {
    const r = await fetch("http://localhost:3000/top_ips")
    const data = await r.json()
    expect(JSON.stringify(data.commonIps)).toEqual(JSON.stringify(['168.41.191.40', '177.71.128.21', '50.112.00.11']))
  })
  it("should return an empty list if count is 0", async function () {
